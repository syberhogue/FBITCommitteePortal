...
export type RichTextEditorHandle = {
  ...
  insertMarkerAtCursor: (marker: string) => string;
};
...
export const RichTextEditor = forwardRef<RichTextEditorHandle, RichTextEditorProps>(
  function RichTextEditor({ name, label, initialValue, compact = false }, ref) {
    ...
    const serializedValue = useCallback(
      () =>
        editor
          ? JSON.stringify(editor.getJSON())
          : serializedInput.current?.value || initialSerialized,
      [editor, initialSerialized],
    );
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        insertMarkerAtCursor: (marker: string) => {
          editor
            ?.chain()
            .focus()
            .insertContent([
              { type: "text", marks: [{ type: "italic" }], text: marker },
              { type: "hardBreak" },
            ])
            .run();
          const value = serializedValue();
          if (serializedInput.current) serializedInput.current.value = value;
          return value;
        },
      }),
